...
namespace asd2
{
    public class City : MarshalByRefObject
    {
        ...
        public bool[] CheckStreetsPairs(Street[] streets, int[] streetsToCheck1, int[] streetsToCheck2)
        {
            ...
            var result = new bool[streetsToCheck1.Length];  //both same size?
            ...
            for (int i = 0; i < streetsToCheck1.Length; i++)
            {
                ...
                {
                    if (connectedStreets.Find(street1) != connectedStreets.Find(k))
                    {
                        int r = CheckIntersection(streets[street1], streets[k]);
                        if (r == int.MaxValue)
                            throw new ArgumentException();
                        else if(r != 0)
                            connectedStreets.Union(street1, k);
                    }

                    if (connectedStreets.Find(street2) != connectedStreets.Find(k))
                    {
                        int r = CheckIntersection(streets[street2], streets[k]);
                        if (r == int.MaxValue)
                            throw new ArgumentException();
                        else if (r != 0)
                            connectedStreets.Union(street2, k);
                    }
                }
